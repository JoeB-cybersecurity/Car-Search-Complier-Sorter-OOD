This is my Car Search Aggregator/Sorter program, built on the Eclipse IDE (Java) environment. It collects data from listings specified by the user (keyword/model of car) from Australian sites such as Carsales, Ebay and Gumtree. This program only searches for cars and vehicles.

This program can currently collect the name, image, link and price (including proper conversion to double type of price to sort listings) from all listings that the user specifies (eg. "holden sv6"). They can also specify how many pages the program will process (number of items to be included in search), location, sort method and whether to include buyitnow or auction listings. 

It uses the jsoup library (web scraper) to fill a 3 dimensional (Items) and 2 dimensional array (PriceDouble) for all the item listings specified by user, and when sorted is reorganised into a 2 dimensional (sorted items) and 1 dimensional array (itemPrice). READ PROGRAM COMMENTS TO SEE WHY
